...
export async function getUserPreferences(): Promise<ProfilePreferences | null> {
  try {
    ...
    return {
      // We’ve already guarded above that data.location and data.searchRadius exist,
      // but TS still sees them as possibly undefined, so assert the types here.
      location: userProfile.data.location as LocationData,
      searchRadius: userProfile.data.searchRadius as number,
    };
  } catch (error) {
